...
class BTP(Dataset):
    ...
    def __init__(self, path, slot_map = None, mode = "intent", mlb = None):
        ...
        for entry in data:
            temp = entry.split("\n")
            self.labels.append(temp[-1].split("#")[0])
    ...
    def __getitem__(self, index, return_sentence = False, pad = False):
        ...
        if return_sentence:
            return sentence, torch.tensor(sentence, dtype=torch.long), target
        return torch.tensor(sentence, dtype=torch.long), target
