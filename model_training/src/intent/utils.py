    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for i, (ids, masks, labels) in enumerate(self.train_loader):
                ...
                # labels  B* (#classes)
                loss = 0
                out_lbl, con_out = self.model(ids, masks) # B * (#classes)
                loss = self.criterion(out_lbl, labels)
                loss += self.cont_loss_contri * self.contrastive_loss(con_out, labels)
    ...
    def calculate_loss_accuracy(self, loader):
        ...
        with torch.no_grad():
            for i, (ids, masks, labels) in enumerate(loader):
                ...
                # labels  B* (#classes)
                loss = 0
                pred_classes, con_out = self.model(ids, masks) # B * (#classes)
                ...
                #     correct += sum((pred_classes[i][true_classes] > 0.5))
